...
# File:         Journal_Constraint_EM_REDD
...
#######################################################################################################################
"""
This function calculates the results for the constrained elastic matching using MVM and the all loads for the REDD-2
dataset. The approach uses the full sampling resolution of 1/3 Hz and active power as input feature, the model size is
1% of the total data. The average accuracy metric for 10-fold cross-validation are (identical as reported in Table 12
of Sub-Section V-A):
...
TECA:   91.09 %
MAE:    3.00
SAE:    0.01 (due to displaying only 2 decimal digits -> 0.016)
"""
...
# Names
# ------------------------------------------
setupExp['name'] = 'Constraint_EM_REDD_ALL'                                                                             # Name of the simulation
...
setupDat['threshold'] = 25                                                                                              # 0) no threshold x) threshold to transform regressio into classification data
...
setupPar['outseq'] = 15                                                                                                 # 0) seq2point, x) length of the subsequence in samples
...
setupPar['outMax'] = 10000                                                                                              # limited output values (maximum)
